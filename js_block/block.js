(() => {
  ...
  class Player extends Rectangle {
    ...
    draw(g) {
      DrawBlock(g, this.mX, this.mY, "#0ff");
    }
    start() {
      this.pCX = WIDTH / 2;
    ...
    tick() {
      this.mX = Math.max(MESH, this.mX - gKey[37] * MAG * 3);
      this.mX = Math.min(WIDTH - MESH * 2, this.mX + gKey[39] * MAG * 3);
      // this.mY = Math.max(MESH, this.mY - gKey[38] * MAG * 2);
      // this.mY = Math.min(HEIGHT - MESH * 2, this.mY + gKey[40] * MAG * 2);
    }
  }

  let gPlayer = new Player();
